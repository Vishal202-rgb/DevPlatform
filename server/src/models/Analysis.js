...
const issueSchema = new mongoose.Schema(
  {
    ...
    // "Apply Fix" tracking (Part 5) - a fix is committed to a NEW branch,
    // never directly to the default branch, so the user always reviews on
    // GitHub before merging.
    fixStatus: {
      type: String,
      enum: ['none', 'applied', 'failed'],
      default: 'none',
    },
    fixBranch: {
      type: String,
