...
const getAllAnalysesForUser = async (userId, limit = 50) => {
  return Analysis.find({ user: userId })
    .sort({ createdAt: -1 })
    .limit(limit)
    .select('-issues') // lightweight list; full detail lives on the per-repo analysis page
    .populate('repository', 'fullName htmlUrl');
};
...
/**
 * Apply one issue's fix: fetch the current file from GitHub, ask Gemini for
 * a full corrected version, commit it to a NEW branch (never the default
 * branch), and record the result on the issue subdocument. The user reviews
 * and opens a PR themselves on GitHub via the returned compare URL.
 */
